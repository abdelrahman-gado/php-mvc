...
class MVCTemplateViewer implements TemplateViewerInterface
{
    ...
    {
        $viewDir = dirname(__DIR__, 2) . '//views//';
        $code = file_get_contents($viewDir . $template);
        if (preg_match('#^{% extends "(?<template>.*)" %}#', $code, $matches)) {
            $base = file_get_contents($viewDir . $matches['template']);
            $blocks = $this->getBlocks($code);
            $code = $this->replaceYields($base, $blocks);
        }

        $code = $this->loadIncludes($viewDir, $code);
    ...
    {
        preg_match_all(
            "#{% block (?<name>\w+) %}(?<content>.*?){% endblock %}#s",
            $code,
            $matches,
            PREG_SET_ORDER,
        );
